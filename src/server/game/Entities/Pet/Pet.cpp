...
{ 
    ...
    {
        SetCreateHealth(NERF_FACTOR*pInfo->health);
        SetModifierValue(UNIT_MOD_HEALTH, BASE_VALUE, NERF_FACTOR*(float)pInfo->health);
    ...
    else                                            // not exist in DB, use some default fake data
    {
        ...
        SetCreateHealth(std::max<uint32>(1, NERF_FACTOR*(stats->BaseHealth[cinfo->expansion]*factorHealth)));
        SetModifierValue(UNIT_MOD_HEALTH, BASE_VALUE, NERF_FACTOR*GetCreateHealth());
    ...
    switch (petType)
    {
        case HUNTER_PET:
        {
            SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, NERF_FACTOR*float(petlevel - (petlevel / 4)));
            SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, NERF_FACTOR*float(petlevel + (petlevel / 4)));
        ...
        case SUMMON_PET:
        {
            ...
            if (pInfo)
            {
                SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, NERF_FACTOR*float(pInfo->min_dmg));
                SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, NERF_FACTOR*float(pInfo->max_dmg));
            }
            else
            {
                SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, NERF_FACTOR*float(petlevel - (petlevel / 4)));
            ...
            switch(GetEntry())
            {
                case NPC_FELGUARD:
                {
                    ...
                    if (AuraEffect* aurEff = m_owner->GetAuraEffectDummy(SPELL_GLYPH_OF_FELGUARD))
                        SetModifierValue(UNIT_MOD_ATTACK_POWER, TOTAL_PCT, NERF_FACTOR*(1.0f + float(aurEff->GetAmount() / 100.0f)));
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case NPC_INFERNAL:
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, NERF_FACTOR*lowAmt*lowAmt*lowAmt);
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, NERF_FACTOR*highAmt*highAmt*highAmt);
