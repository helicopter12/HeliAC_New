...
void WorldSession::SendListInventoryT1(uint64 vendorGUID, uint64 vendorWindowID)
{
	// remove fake death
	if (GetPlayer()->HasUnitState(UNIT_STATE_DIED))
		GetPlayer()->RemoveAurasByType(SPELL_AURA_FEIGN_DEATH);

	VendorItemData const* items = sObjectMgr->GetNpcVendorItemList(vendorWindowID);
	if (!items)
	{
